...
class subcategory extends Front_Controller
{
    ...
    public function get_by_category_name()
    {
        $category_name = $this->input->get('category_name');
        $data_fetch_subcategory=$this->subcategory_model->get_by_category_name($category_name);
        $subcategory_list=array();
        foreach ($data_fetch_subcategory as $row)
        {
            $subcategory_list[$row->sub_category_name]=$row->sub_category_name;
        }
