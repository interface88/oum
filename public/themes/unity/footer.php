<footer id="footer">
    <div class="footer-top">
      <div class="container">
        <div class="row">
          <div class="col-lg-2 col-md-6 footer-links">
            <h4>Company</h4>
            <ul>
              <li><i class="bx bx-chevron-right"></i> <a href="#">About us</a></li>
              <li><i class="bx bx-chevron-right"></i> <a href="#">Join us</a></li>
              <li><i class="bx bx-chevron-right"></i> <a href="#">Offices</a></li>
              <li><i class="bx bx-chevron-right"></i> <a href="#">Privacy & policy</a></li>
              <li><i class="bx bx-chevron-right"></i> <a href="#">Sitemap</a></li>
              <li><i class="bx bx-chevron-right"></i> <a href="#">Contact</a></li>
            </ul>
          </div>

          <div class="col-lg-3 col-md-6 footer-links">
            <h4>Categories</h4>
            <ul>
              <li><i class="bx bx-chevron-right"></i> <a href="#">Medical</a></li>
              <li><i class="bx bx-chevron-right"></i> <a href="#">Family</a></li>
              <li><i class="bx bx-chevron-right"></i> <a href="#">Children</a></li>
              <li><i class="bx bx-chevron-right"></i> <a href="#">Education</a></li>
              <li><i class="bx bx-chevron-right"></i> <a href="#">Disaster</a></li>
            </ul>
          </div>
	
	<div class="col-lg-3 col-md-6">
            <div class="footer-info">
              <h4>HEAD OFFICE</h4>
              <p>
                <strong>Address:</strong> 8 King Street, Melbourne Victoria 89<br>
                <strong>Phone:</strong>+22 6 4567 8900<br>
              </p>
            </div>
            <div class="footer-info">
              <h4>Brand</h4>
              <p>
                <strong>Address:</strong> 2 Queen Street,California, USA<br>
                <strong>Phone:</strong>+22 6 4567 8900<br>
              </p>
            </div>
          </div>

          <div class="col-lg-4 col-md-6 footer-newsletter">
            <h4>Our Newsletter</h4>
            <p>Subscribe to receive inspiration, ideas, and news in your inbox.</p>
            <form action="" method="post">
              <input type="email" name="email"><input type="submit" value="Subscribe">
            </form>

          </div>

        </div>
      </div>
    </div>
	<div class="footer-bottom">
        <div class="container">
         <div class="row">
         	 <div class="col-lg-6 col-md-6">
                  <div class="copyright">
                    &copy; Copyright <strong><span>OUM</span></strong>. All Rights Reserved
                  </div>
         	 </div>
         	 <div class="col-lg-6 col-md-6">
                  <div class="social-links float-right">
                    <a href="#" class="twitter"><i class="bx bxl-twitter"></i></a>
                    <a href="#" class="facebook"><i class="bx bxl-facebook"></i></a>
                    <a href="#" class="instagram"><i class="bx bxl-instagram"></i></a>
                    <a href="#" class="google-plus"><i class="bx bxl-skype"></i></a>
                    <a href="#" class="linkedin"><i class="bx bxl-linkedin"></i></a>
                  </div>
         	 </div>
         </div>
        </div>
	</div>
  </footer><!-- End Footer -->